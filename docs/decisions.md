...
### D104 dis-ui-server gains a platform-scoped write to `atlas.schema_drafts` (Atlas A4 PR2) `RESOLVED`

**The decision.** The Atlas console persists draft and frozen canonical-schema IRs in a new `atlas.schema_drafts` table (a new `atlas` schema, migration 0013), written by dis-ui-server. This expands the BFF write scope beyond `config.source_mappings` (D26). The table is **platform-scoped and non-RLS** (no `tenant_id`, no policy; the `identity_mirror` precedent, D36/D41): an Atlas draft is cross-tenant Super-Admin state, not tenant-private data. Writes go through `rls_platform_session` (PLATFORM); the GUC is inert on a policy-less table (the session's "writes nothing" is the tenant-scoped WITH CHECK on RLS tables, absent here). The IR is stored as a JSONB document via the single `schema_to_document(SchemaIR)` serializer and round-tripped with `parse_ir` (no second IR shape). A published row is immutable, enforced both by the store contract (`update` refuses a non-draft; `freeze` is the sole draft->published transition and runs the ratify gate) AND a DB trigger plus a partial unique index on `(vertical, table_key, schema_version) WHERE status='published'`. `'superseded'` is in the status CHECK but reserved for A6 (evolution/diff); no PR2 code path writes it.

**Ledger boundary (ADR-ATLAS-001 decision 6 / A5).** This table is the **artifact store** (the frozen IR + status + version), which is DIS's concern. The **publish audit/action ledger** (who published what, when) remains Customer Master's (decision 6, A5), behind the `emit_atlas_publish` seam. The `published_at` and `created_by_user_id` columns are **operational metadata, not the governance ledger** — nothing may present them as the audit record.

**Cross-refs.** D26 (BFF write scope), D36/D41 (identity_mirror non-RLS), D99 (downgrade-defer), ADR-ATLAS-001 decision 6, build plan A4/A5.
